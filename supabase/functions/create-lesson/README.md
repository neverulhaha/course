...
## Важный фикс для фронтенда

Ошибка `new row violates row-level security policy for table "lessons"` означает, что фронтенд всё ещё делает прямой insert в таблицу `lessons` через PostgREST/Supabase client:

```ts
await supabase.from("lessons").insert(...)
```

Для безопасного создания урока фронтенд должен вызывать Edge Function:
...
В этом архиве проект уже исправлен: `src/services/courseEditor.service.ts` теперь создаёт урок через `create-lesson`, а не через прямой `insert` в `lessons`.
