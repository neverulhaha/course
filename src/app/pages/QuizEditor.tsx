...
import { fetchQuizForEditor, type QuizQuestionView } from "@/services/coursePlayback.service";
...
export default function QuizEditor() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void (async () => {
      const res = await fetchQuizForEditor(quizId);
      if (cancelled) return;
      if (res.error) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#F9FAFB] to-white flex items-center justify-center">
        <p className="text-sm text-gray-600">Загрузка квиза…</p>
      </div>
    );
  }

  if (loadError && !hasQuiz) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#F9FAFB] to-white flex flex-col items-center justify-center gap-3 px-6 text-center">
        <p className="text-sm text-gray-700">Не удалось загрузить квиз.</p>
        <Link to={`/app/editor/${courseId}`} className="text-[#4A90E2] font-semibold text-sm">
          Назад
        </Link>
      </div>
    );
        ...
        <div className="max-w-5xl mx-auto px-8 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">{title || "Квиз"}</h1>
