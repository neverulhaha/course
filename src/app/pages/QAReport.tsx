...
export default function QAReportPage() {
  ...
  const scoreItems = useMemo(() => {
    return SCORE_LABELS.filter((score) => score.key !== "source_alignment_score" || latest?.qa_scope?.source_alignment_enabled === true || latest?.source_alignment_score !== null);
  }, [latest]);
    ...
    <main className="min-h-screen bg-[var(--bg-page)] px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
        ...
        {!latest ? (
          ...
          <>
            <section className="grid gap-4 lg:grid-cols-[1.05fr_1.5fr]"><OverallScoreCard report={latest} /><div className="grid gap-4 sm:grid-cols-2">{scoreItems.map((score) => <ScoreCard key={score.key} label={score.label} description={score.description} value={latest[score.key]} />)}</div></section>
            ...
            {latest.qa_scope && <ScopeNotice scope={latest.qa_scope} />}
...
function OverallScoreCard({ report }: { report: QaReport }) { return <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-col gap-5 sm:flex-row sm:items-center"><div className="flex size-36 shrink-0 items-center justify-center rounded-full p-3" style={{ background: scoreGradient(report.total_score) }} aria-label={`Общий балл качества: ${scoreText(report.total_score)}`}><div className="flex size-full flex-col items-center justify-center rounded-full bg-white text-center shadow-inner"><span className={cn("text-4xl font-black", scoreClass(report.total_score))}>{scoreText(report.total_score)}</span><span className="text-xs font-bold uppercase tracking-wide text-slate-400">из 100</span></div></div><div><h2 className="text-xl font-bold text-slate-950">Общий балл качества</h2><p className="mt-2 text-sm leading-6 text-slate-500">Итоговая оценка формируется только по доступным на текущий момент материалам. Источники учитываются только для курсов, созданных по пользовательскому источнику.</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100"><div className={cn("h-full rounded-full", scoreBarClass(report.total_score))} style={{ width: `${Math.max(0, Math.min(100, report.total_score ?? 0))}%` }} /></div></div></div></article>; }
function ScopeNotice({ scope }: { scope: QaScopeInfo }) { const isCurrent = scope.mode === "current"; const isPlan = scope.mode === "plan"; return <section className={cn("rounded-3xl border p-5 text-sm shadow-sm", isCurrent || isPlan ? "border-blue-200 bg-blue-50 text-blue-900" : "border-emerald-200 bg-emerald-50 text-emerald-900")}><div className="flex items-start gap-3"><div className={cn("mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-2xl", isCurrent || isPlan ? "bg-blue-100" : "bg-emerald-100")}>{isCurrent || isPlan ? <AlertTriangle className="size-5" /> : <CheckCircle2 className="size-5" />}</div><div><h2 className="font-black text-slate-950">Область проверки</h2><p className="mt-1 leading-6">{scope.message}</p>{scope.missing_content_lessons_count > 0 && <p className="mt-2 font-semibold">Нужно добавить содержимое ещё для уроков: {scope.missing_content_lessons_count}.</p>}{!scope.source_alignment_enabled && <p className="mt-2 text-xs font-semibold uppercase tracking-wide opacity-80">Проверка источников отключена: курс не создан по источнику.</p>}</div></div></section>; }
