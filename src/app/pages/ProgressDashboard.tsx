...
function mapCourseProgress(c: CourseProgressView): CourseProgress {
  return {
    id: c.id,
    title: c.title,
    progress: c.progress,
...
function mapActivity(a: ActivityView): ActivityItem {
  return {
    type: a.type,
    title: a.title,
    course: a.course,
...
function NextStepCard({ courses }: { courses: CourseProgress[] }) {
    ...
    return (
      ...
      >
        ...
        <p className="text-[12px]" style={{ color: "var(--gray-500)" }}>
          Пока нет записей о прохождении уроков и квизов.
        </p>
      </div>
    );
  }
...
export default function ProgressDashboard() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void (async () => {
      const res = await fetchStudentProgressDashboard(user.id);
      if (cancelled) return;
      if (res.error) {
      ...
      setLoadError(null);
      setCourses(res.courses.map(mapCourseProgress));
      setActivity(res.activity.map(mapActivity));
      setStats(res.stats);
    })();
    return () => {
      cancelled = true;
    ...
    () =>
      [
        ...
        },
        {
          emoji: "⚡",
          title: "Отличник",
          description: "Средний балл по квизам выше 80%",
          earned: stats.averageScore != null && stats.averageScore >= 80,
        },
        {
          emoji: "🔥",
          title: "Серия занятий",
      ...
      >
        <div className="mx-auto flex max-w-[1600px] flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
          <div className="min-w-0">
            ...
            {loadError && <p className="mt-1 text-[11px] text-red-600">Ошибка загрузки: {loadError}</p>}
          </div>
