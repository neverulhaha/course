...
type CourseStatus = "ready" | "has-issues" | "generated" | "draft";
...
> = {
  ready:       { label: "Готово",          dot: "#2ECC71", color: "#2ECC71", bg: "rgba(46,204,113,0.07)"  },
  "has-issues":{ label: "Есть замечания",  dot: "#F1C40F", color: "#F1C40F", bg: "rgba(241,196,15,0.07)" },
  generated:   { label: "Сгенерировано",   dot: "#4A90E2", color: "#4A90E2", bg: "rgba(74,144,226,0.07)" },
  draft:       { label: "Черновик",        dot: "#8E9BAB", color: "#5C6B7A", bg: "rgba(142,155,171,0.08)"},
};
...
function QaChip({ score }: { score: number }) {
  const color =
    score >= 85 ? "#2ECC71" :
...
function SummaryStrip({ courses }: { courses: Course[] }) {
  const inProgress = courses.filter(
    (c) => c.status === "has-issues" || c.status === "generated" || c.progress < 100
  ).length;
  const ready = courses.filter((c) => c.status === "ready").length;
  const avgQa = Math.round(courses.reduce((s, c) => s + c.qaScore, 0) / (courses.length || 1));
...
export default function Dashboard() {
  ...
  // Recent = sorted by index (first 2 most-recent-looking)
