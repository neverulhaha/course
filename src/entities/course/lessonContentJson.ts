...
export const emptyLessonContent: LessonContent = { id: null, goal: null, blocks: [] };
...
export function parseLessonContentRow(raw: unknown, fallbackGoal?: string | null): LessonContent {
  ...
  if (!r) return { id: null, goal: fallbackGoal ?? null, blocks: [] };
  if (r.content != null) {
    const parsed = parseLessonContentJson(r.content, fallbackGoal);
    return { ...parsed, id: str(r.id) ?? parsed.id ?? null };
  }
  ...
  return { id: str(r.id) ?? null, goal: str(r.goal) ?? fallbackGoal ?? null, blocks };
}
...
export function parseLessonContentJson(raw: unknown, fallbackGoal?: string | null): LessonContent {
  ...
  if (!rec) return { id: null, goal: fallbackGoal ?? null, blocks: [] };
  ...
  if (!Array.isArray(blocksRaw)) return { id: str(rec.id) ?? null, goal, blocks: [] };
  const blocks: LessonBlock[] = [];
  ...
  return { id: str(rec.id) ?? null, goal, blocks };
}
